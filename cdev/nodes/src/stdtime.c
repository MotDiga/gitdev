...
void mtime_arr(mtime_t *mtime, real64_t sec, int64_t min, int64_t hour, int64_t day, int64_t mth, int64_t year)
{
    ...
    default:
        ...
        if ((days >= _Feb_days+1) && IS_LEAP_YEAR(mtime->year))
        {
            days -= _Feb_days+1;
            mtime->mth++;
        }
        else if ((days >= _Feb_days))
        {
            days -= _Feb_days;
            mtime->mth++;
        }
...
uint64_t mtime_64seed()
{
    msec_t msec;
    timespec_get((struct timespec*)&msec, TIME_UTC);
    // uint64_t seed = msec.nsec + msec.sec*1000000000UL;

    // return seed;
    return msec.nsec + msec.sec*1000000000UL;
}
...
inline real64_t mtime_delta(mtime_t mtime0, mtime_t mtime1)
{
