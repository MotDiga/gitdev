...
#include <sandbox3/sandboxresources.h>
#include <renderer/renderer.h>


//********************************************
renderer_t _renderer;
#define INITWINDTH 960.0f
#define INITHEIGHT 540.0f
#define INITXPOS INITWINDTH/2.0f
...
int32_t sandbox(int32_t argc, string_t argv[])
{
    ...
    window_t *window   = glfwCreateWindow(INITWINDTH, INITHEIGHT, "LearnOpenGL", monitor, NULL);
    if (window == NULL)
    {
        glfwTerminate();
        LOG_FATAL(EXIT_FAILURE, "unable to init glfw")
    ...
    uint32_t square3D_vao;
    glGenVertexArrays(1, &square3D_vao);
    glBindVertexArray(square3D_vao);
    uint32_t square3D_vbo;
    glGenBuffers(1, &square3D_vbo);
    glBindBuffer(GL_ARRAY_BUFFER, square3D_vbo);
    glBufferData(GL_ARRAY_BUFFER, sizeof(square3D), (real32_t*)square3D, GL_STATIC_DRAW);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(real32_t), (void*)0);
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(real32_t), (void*)(3 * sizeof(real32_t)));
    glEnableVertexAttribArray(1);
    uint32_t square3D_ebo;
    glGenBuffers(1, &square3D_ebo);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, square3D_ebo);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(square3Dindeces), (uint32_t*)square3Dindeces, GL_STATIC_DRAW);
    geometrycounts++;
    ...
    texture_2dgen(&textures[4], "rcs/files/mipmap.png",           4, GL_NEAREST_MIPMAP_LINEAR, GL_NEAREST_MIPMAP_NEAREST, GL_CLAMP_TO_BORDER, GL_CLAMP_TO_BORDER);
    texturecounts += 5;
...
#define FOVP_MIN -0.897875634648741
    ...
    vec3_t camera_pos = { 0.0f, 0.0f, tanf((M_PI - fov)/2.0f)}, camera_target = { 0.0f, 0.0f, 0.0f };
    vec3_t vec3_up = { 0.0f, 1.0f, 0.0f };
    vec3_t camera_direction, camera_right, camera_up;
    glm_vec3_sub(camera_pos, camera_target, camera_direction);
    glm_normalize(camera_direction);
    glm_vec3_cross(vec3_up, camera_direction, camera_right);
    glm_normalize(camera_right);
    glm_vec3_cross(camera_right, camera_direction, camera_up);
    // glm_mat4_copy((mat4_t){{  camera_right[0],  camera_up[0] , -camera_direction[0], 0.0f },
    //                        {  camera_right[1],  camera_up[1] , -camera_direction[1], 0.0f },
    //                        {  camera_right[2],  camera_up[2] , -camera_direction[2], 0.0f },
    //                        { -camera_pos[0]  , -camera_pos[1],  camera_pos[2]      , 1.0f }}, view);
    glm_lookat(camera_pos, camera_target, vec3_up, view);
    // printf("        _                                  _\n");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm_translate(view, (vec3_t){ (camera_gox[_LEFT] - camera_gox[_RIGHT]) * _renderer.dtime, (camera_goy[_LEFT] - camera_goy[_RIGHT]) * _renderer.dtime, (camera_goz[_FORWARD] - camera_goz[_BACK]) * _renderer.dtime });
        ...
        glm_translate(model, (vec3_t){ (model_gox[_RIGHT] - model_gox[_LEFT]) * _renderer.dtime, (model_goy[_UP] - model_goy[_DOWN]) * _renderer.dtime, (model_goz[_FORWARD] - model_goz[_BACK]) * _renderer.dtime });
    ...
    IFDEBUG (
        ...
        // printf("Maximum number of vertex attributes supported: %lu\n", attributes);
        // glGetIntegerv(GL_MAX_TEXTURE_IMAGE_UNITS, &attributes);
        // printf("Maximum number of texture image units supported: %lu\n", attributes);
    )
    texture_free(&textures[0]);
    texture_free(&textures[1]);
    texture_free(&textures[2]);
    texture_free(&textures[3]);
    texture_free(&textures[4]);
...
static void key_callback(window_t *window, int32_t key, int32_t scancode, int32_t action, int32_t mods)
{
    ...
    if (key == GLFW_KEY_UP && action == GLFW_PRESS)              { model_goy[_FORWARD] = 1.29f; }
    if (key == GLFW_KEY_UP && action == GLFW_RELEASE)            { model_goy[_FORWARD] = 0.0f; }
    if (key == GLFW_KEY_DOWN && action == GLFW_PRESS)            { model_goy[_BACK] = 1.29f; }
    if (key == GLFW_KEY_DOWN && action == GLFW_RELEASE)          { model_goy[_BACK] = 0.0f; }
    if (key == GLFW_KEY_RIGHT_CONTROL && action == GLFW_PRESS)   { model_goz[_UP] = 0.75f; }
    if (key == GLFW_KEY_RIGHT_CONTROL && action == GLFW_RELEASE) { model_goz[_UP] = 0.0f; }
    if (key == GLFW_KEY_RIGHT_SHIFT && action == GLFW_PRESS)     { model_goz[_DOWN] = 0.75f; }
    if (key == GLFW_KEY_RIGHT_SHIFT && action == GLFW_RELEASE)   { model_goz[_DOWN] = 0.0f; }
    //* Model Rotation
...
void scroll_callback(window_t *window, real64_t xoffset, real64_t yoffset)
{
...
static void mousebutton_callback(window_t *window, int32_t button, int32_t action, int32_t mods)
{
    ...
    // }
    //* Switch Texture
    if (button == GLFW_MOUSE_BUTTON_4 && (action == GLFW_PRESS || action == GLFW_REPEAT))
    {
        texture = (texture + 1) % texturecounts;
        texturelock = false;
    }
    if (button == GLFW_MOUSE_BUTTON_5 && (action == GLFW_PRESS || action == GLFW_REPEAT))
    {
        texture = ((texture + texturecounts) - 1) % texturecounts;
        texturelock = false;
    }
    //* Switch Polygone Mode
    if (button == GLFW_MOUSE_BUTTON_3 && (action == GLFW_PRESS || action == GLFW_REPEAT))
    {
        if (polygonemode == GL_LINE)
        {
            glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
