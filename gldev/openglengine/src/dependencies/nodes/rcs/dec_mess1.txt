...
uint32_t rsa_find_c_m_dlist(const uint64_t m_c, const uint64_t e_d, const uint64_t n)
{
    ...
    for (uint64_t i = 0, last_m_c = -1; last_m_c != m_c; i++)   // calculate the periode at witche: (m_c^periode)%n = m_c%n
    {
...
rsakey_t rsa_genkeyfile(const uint32_t p, const uint32_t q)
{
    FILE *rsakey = fopen("key.rsa", "w");
    if (rsakey == NULL)
    {
        LOG_FATAL(-1, "unable to open key.rsa");
    }
    rsakey_t key;
    RSA_GENKEY(&key, p, q);
    fprintf(rsakey, "%u\n%u\n%u\ne, d, n\n", key.e, key.d, key.n);
...
void rsa_genkeyfile_fromkey(const rsakey_t key)
{
    FILE *rsakey = fopen("key.rsa", "w");
    if (rsakey == NULL)
    {
...
void rsa_encriptfile(const string_t pubkeyfile, const string_t messagefile, string_t encmessfile)
{
    ...
    {
        LOG_FATAL(-1, "unable to open %s, %s or %s", pubkeyfile, messagefile, encmessfile);
    }
...
void rsa_decriptfile(const string_t privkeyfile, const string_t encmessfile, string_t decmessfile)
{
    ...
    {
        LOG_FATAL(-1, "unable to open %s, %s or %s", privkeyfile, encmessfile, decmessfile);
    }
    ...
    while ((tmp_size = fread(m, sizeof(uchar_t), 4, message)))
    {
        if (tmp_size == 4)
        {
            ...
            IFDEBUG (
                ...
                {
                    LOG_FATAL(-2, "unable to write to file %s", encmessfile);
                }
            )
        }
        else
    ...
    while ((tmp_size = fread(c, sizeof(char32_t), 4, encmess)))
    {
        if (tmp_size == 4)
        {
            ...
            IFDEBUG (
                ...
                {
                    LOG_FATAL(-2, "unable to write to file %s", decmessfile);
                }
            )
        }
        else
